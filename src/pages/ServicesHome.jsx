...
const services = [
    {
        ...
        accentColor: 'text-primary-600 dark:text-primary-300',
    ...
    },
    {
        ...
        cardBg: 'from-amber-50 via-white to-yellow-50 dark:from-amber-500/10 dark:via-gray-950 dark:to-yellow-500/10',
        badge: 'Smart',
        badgeClassName: 'bg-amber-100 text-amber-700 dark:bg-amber-500/20 dark:text-amber-300',
        link: '/split-expense',
        cta: 'Split Now',
    },
    {
...
export default function ServicesHome() {
    return (
        <div className="min-h-screen bg-white dark:bg-gray-950 text-gray-900 dark:text-white transition-colors duration-500">
                    ...
                    <div className="grid items-start gap-6 lg:grid-cols-[1.4fr_0.85fr]">
                        ...
                        <div className="rounded-3xl border border-gray-200/80 bg-gray-50/80 p-6 shadow-sm dark:border-white/[0.08] dark:bg-white/[0.04]">
                            <div className="flex items-center justify-between gap-3">
                                <p className="text-xs font-bold uppercase tracking-[0.22em] text-gray-400 dark:text-gray-500">Jump In</p>
                                <span className="rounded-full border border-gray-200/80 bg-white/80 px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider text-gray-500 dark:border-white/[0.08] dark:bg-white/[0.04] dark:text-gray-400">
                                    6 Services
                                </span>
                            </div>
                            ...
                            <div className="mt-5 grid grid-cols-2 gap-3">
                                {services.map((service) => (
                                    <Link
                                        key={service.id}
                                        to={service.link}
                                        className="group rounded-2xl border border-gray-200/80 bg-white/80 p-3.5 transition-all duration-200 hover:-translate-y-0.5 hover:bg-white hover:shadow-md dark:border-white/[0.08] dark:bg-white/[0.03] dark:hover:bg-white/[0.06]"
                                    >
                                        <div className={`flex h-10 w-10 items-center justify-center rounded-2xl bg-gradient-to-br ${service.iconGradient} text-white shadow-sm`}>
                                            {service.icon}
                                        </div>
                                        <p className="mt-3 text-sm font-bold tracking-tight text-gray-900 dark:text-white">{service.name}</p>
                                        <p className="mt-1 text-[11px] leading-tight text-gray-500 dark:text-gray-400">{service.tagline}</p>
                                    </Link>
                                ))}
                            </div>
                        </div>
                ...
                <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
                    {services.map((service) => (
                        <Link
                            key={service.id}
                            to={service.link}
                            className={`group flex flex-col rounded-3xl border border-gray-200/80 bg-gradient-to-br ${service.cardBg} p-6 shadow-xl shadow-gray-100/40 transition-all duration-300 hover:-translate-y-1 hover:shadow-2xl dark:border-white/[0.08] dark:shadow-none`}
                        >
                            {/* Icon + Badge */}
                            <div className="flex items-start justify-between mb-5">
                                <div className={`flex h-12 w-12 items-center justify-center rounded-2xl bg-gradient-to-br ${service.iconGradient} text-white shadow-lg shadow-black/5`}>
                                    {service.icon}
                                </div>
                                {service.badge && (
                                    <span className={`text-[10px] font-bold uppercase tracking-wider px-2 py-0.5 rounded-full ${service.badgeClassName}`}>
                                        {service.badge}
                                    </span>
                                )}
                            </div>
                            ...
                            <p className={`text-xs font-bold uppercase tracking-[0.2em] mb-3 ${service.accentColor}`}>{service.tagline}</p>
                            ...
                            {/* CTA */}
                            <span className={`inline-flex items-center gap-1.5 text-sm font-semibold transition-all duration-200 group-hover:gap-3 ${service.accentColor}`}>
                                {service.cta}
                                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <section className="max-w-7xl mx-auto px-4 pb-16 sm:px-6 lg:px-8">
                <div className="rounded-3xl border border-gray-200/80 bg-gray-50/80 p-6 shadow-sm dark:border-white/[0.08] dark:bg-white/[0.04]">
                    <p className="text-xs font-bold uppercase tracking-[0.22em] text-gray-400 dark:text-gray-500">Platform Highlights</p>
                    <div className="mt-5 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                        {platformFeatures.map((feature) => (
                            <div
                                key={feature.title}
                                className="rounded-2xl border border-gray-200/80 bg-white/80 p-4 shadow-sm dark:border-white/[0.08] dark:bg-white/[0.03]"
                            >
                                <div className={`flex h-10 w-10 items-center justify-center rounded-2xl ${feature.iconBg} ${feature.iconColor}`}>
                                    {feature.icon}
                                </div>
                                <h3 className="mt-4 text-sm font-bold text-gray-900 dark:text-white">{feature.title}</h3>
