        ...
        <div className="min-h-screen overflow-hidden bg-stone-950 text-stone-100">
            ...
            <main className="relative mx-auto flex min-h-screen max-w-6xl items-center px-4 py-12 sm:px-6 lg:px-8">
                <div className="grid w-full gap-8 lg:grid-cols-[1.2fr_0.8fr]">
                    ...
                    <aside className="rounded-[2rem] border border-white/10 bg-stone-900/70 p-8 shadow-2xl shadow-black/20 backdrop-blur-2xl sm:p-10">
                        <p className="text-xs font-bold uppercase tracking-[0.24em] text-stone-500">Status Board</p>
                        <div className="mt-5 space-y-4">
                            <div className="rounded-3xl border border-white/10 bg-white/[0.04] p-5">
                                <p className="text-sm font-semibold text-white">Why this page helps</p>
                                <p className="mt-2 text-sm leading-relaxed text-stone-400">
                                    Visitors get a clear answer immediately instead of running into broken forms, failed saves, or partial database updates.
                                </p>
                            </div>
                            <div className="rounded-3xl border border-white/10 bg-white/[0.04] p-5">
                                <p className="text-sm font-semibold text-white">Recommended workflow</p>
                                <p className="mt-2 text-sm leading-relaxed text-stone-400">
                                    Turn maintenance mode on before schema work, deploy, run your database changes, verify a key flow, then switch it off.
                                </p>
                            </div>
                            <div className="rounded-3xl border border-white/10 bg-white/[0.04] p-5">
                                <p className="text-sm font-semibold text-white">Best next upgrade</p>
                                <p className="mt-2 text-sm leading-relaxed text-stone-400">
                                    If you want instant toggles later, move the flag to a runtime config source instead of relying only on frontend build-time env values.
                                </p>
                            </div>
                        </div>
                    </aside>
