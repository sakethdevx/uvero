 ...
 * Two modes: Share (paste text → get code) and Retrieve (enter code → get text).
 */
export default function ClipboardQuickPanel({ params, onDismiss }) {
  const [mode, setMode] = useState('share'); // 'share' | 'retrieve'
      ...
      <div className="flex gap-1 p-1 rounded-xl" style={{ background: 'var(--surface-2)' }}>
        <button
          ...
          className={`flex-1 text-xs font-semibold py-2 rounded-lg transition-all ${
            mode === 'share' ? 'text-white shadow-sm' : ''
          }`}
        ...
        <button
          ...
          className={`flex-1 text-xs font-semibold py-2 rounded-lg transition-all ${
            mode === 'retrieve' ? 'text-white shadow-sm' : ''
          }`}
        ...
        retrievedText ? (
          <div className="animate-panel-in space-y-3">
            ...
            <div className="flex gap-2">
              <button onClick={copyRetrieved} className="btn-accent flex-1 text-sm">
                {copied ? '✓ Copied' : 'Copy Text'}
              </button>
              <button
                onClick={() => { setRetrievedText(''); setCode(''); }}
