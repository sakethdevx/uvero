...
export default async function handler(req, res) {
    ...
    try {
        ...
        if (actor.type !== 'guest') {
            throw createHttpError(403, 'This recovery path is for guest mode only')
        }
        ...
        const { data: existingMembership, error: existingMembershipError } = await supabase
            ...
            .select('*')
            .eq('group_id', group.id)
            .eq('guest_session', actor.guest_session)
            .maybeSingle()
        ...
        if (shouldClaimOwnership) {
            const { error: claimGroupOwnerError } = await supabase
                ...
                .update({
                    ...
                    created_by_guest_session: actor.guest_session,
                    is_guest_group: true
                })
                .eq('id', group.id)
        ...
        const { data: consumeRow, error: consumeCodeError } = await supabase
            .from('split_group_recovery_codes')
            .update({
        ...
        if (!consumeRow) {
            throw createHttpError(409, 'Recovery code was already consumed')
        }
        ...
        return sendApiError(res, error, '[api/split/recover-guest]')
    }
}
