...
export default function LanguageSelector({ selectedLanguage, onLanguageChange, variant = 'default' }) {
    const [isOpen, setIsOpen] = useState(false);
    const [search, setSearch] = useState('');
    const dropdownRef = useRef(null);
    const searchRef = useRef(null);

    const isStatusBar = variant === 'status-bar';
        ...
        <div className="relative" ref={dropdownRef}>
            ...
            <button
                ...
                className={`flex items-center transition-all duration-300 group ${
                    isStatusBar 
                        ? `gap-1.5 px-2 py-0.5 rounded hover:bg-gray-200/50 dark:hover:bg-white/5 ${isOpen ? 'text-blue-500 dark:text-blue-400' : 'text-gray-500 dark:text-gray-500'}`
                        : `gap-3 px-4 py-2.5 rounded-xl border shadow-sm ${
                            isOpen 
                                ? 'bg-white dark:bg-white/10 border-blue-500/50 ring-4 ring-blue-500/10' 
                                : 'bg-white/80 dark:bg-white/[0.04] border-gray-200 dark:border-white/10 hover:border-gray-300 dark:hover:border-white/20 hover:bg-white dark:hover:bg-white/10'
                        }`
                }`}
            >
                <span className={`${isStatusBar ? 'text-sm' : 'text-xl group-hover:scale-110'} transition-transform duration-300`}>
                    {current?.icon || '💻'}
                </span>
                
                {!isStatusBar ? (
                    <div className="text-left">
                        <p className="text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-widest leading-none mb-1">Language</p>
                ...
                <svg className={`ml-1 w-3 h-3 text-gray-400 transition-transform duration-300 ${isOpen ? (isStatusBar ? '-rotate-180' : 'rotate-180') : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2.5" d={isStatusBar ? "M5 15l7-7 7 7" : "M19 9l-7 7-7-7"} />
                </svg>
            </button>

            {/* Dropdown */}
            {isOpen && (
                <div className={`absolute right-0 ${isStatusBar ? 'bottom-full mb-2 origin-bottom-right' : 'top-full mt-3 origin-top-right'} lg:left-0 w-80 bg-white/95 dark:bg-[#0d1117]/95 backdrop-blur-xl border border-gray-200 dark:border-white/10 rounded-2xl shadow-2xl shadow-black/20 z-50 overflow-hidden animate-pop-in ${!isStatusBar && 'lg:origin-top-left'}`}>
                    {/* Search */}
                    ...
                    {/* Language List */}
                    <div className="max-h-[300px] overflow-y-auto py-2 custom-scrollbar text-left">
                        {filtered ? (
                            // Search results
                            filtered.length > 0 ? (
                                <div className="px-2">
